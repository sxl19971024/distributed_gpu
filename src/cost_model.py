...
class SplitStrategy(Enum):
    """分割策略类型"""
    ROW_SPLIT = "row"           # 按行分割
    COLUMN_SPLIT = "column"     # 按列分割
    BLOCK_2D = "block_2d"       # 2D块分割
...
class CostModel:
    ...
    def estimate_matmul_cost(self, M: int, K: int, N: int,
        ...
        if strategy == SplitStrategy.ROW_SPLIT:
            ...
            bandwidth = self.config.inter_node_bandwidth * 1e9
            comm_time_ms = (total_comm_volume / bandwidth) * 1000 + self.config.network_latency_ms * 3
    ...
    def print_analysis(self, M: int, K: int, N: int):
        ...
        for strategy in [SplitStrategy.ROW_SPLIT, SplitStrategy.COLUMN_SPLIT]:
            cost = self.estimate_matmul_cost(M, K, N, strategy)
            status = "✓" if cost.feasible else "✗"
            print(f"{strategy.value:12s} | 计算: {cost.compute_time_ms:8.2f}ms | "
        ...
        print(f"\n推荐策略: {best.strategy.value}")
        print(f"预估加速比: {best.cost.speedup:.2f}x")
