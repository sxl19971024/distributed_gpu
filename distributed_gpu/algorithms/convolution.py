...
def distributed_conv2d(input: Optional[torch.Tensor],
    ...
    if should_use_single_gpu(mpi, input, weight):
        if mpi.is_master_process():
            return F.conv2d(input, weight, bias, stride=stride,
    ...
    # 所有进程统一判断是否有 bias（避免主进程/非主进程分支不一致导致死锁）
    has_bias = bias is not None if mpi.is_master_process() else None
    has_bias = mpi.broadcast(has_bias)
    bias_local = distributor.broadcast(bias) if has_bias else None
...
def distributed_conv3d(input: Optional[torch.Tensor],
    ...
    if should_use_single_gpu(mpi, input, weight):
        if mpi.is_master_process():
            return F.conv3d(input, weight, bias, stride=stride,
                            padding=padding, dilation=dilation, groups=groups)
        return None
